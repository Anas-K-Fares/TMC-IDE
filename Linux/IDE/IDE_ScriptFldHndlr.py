...
class ScriptFieldHndlr:
    ...
    def populateDevLst(self, coordinates): # parameter coordinates is an assumed output to the bind function and not actually known
        ...
        try:
            ...
            for dev in devsLst:
                if dev != '':
                    self.devicesList.menu.add_command(label = dev, background='white', activebackground='white')
                
        except:
            self.devicesList.menu.add_command(label = "                   Connect               ", background='white', activebackground='white', command=self.connectHndlr)
